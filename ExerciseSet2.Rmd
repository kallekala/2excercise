...
1. 6 components
2. 0,93 and 0,86.
3. Looking at the columns one can see what are the main attributes used for each component.
I would call them
  1. Government bonds
  2. Currencies
  3. Large cap indices Europe
  4: Small cap indices NA
  5: European Fixed Income futures
  6: Oil
  7: Metals
  8: Oilseeds
  9: Precious metals
...
pre_crisis_pca <- PCA(pre_crisis_data, graph=FALSE)
crisis_pca <- PCA(crisis_data, graph=FALSE)

Put into table-form:

pre_crisis_Table<-cbind(paste("component",1:ncol(pre_crisis_data),sep=" "),pre_crisis_pca$eig)
pre_crisis_Table[,1] <- NULL

crisis_Table<-cbind(paste("component",1:ncol(crisis_data),sep=" "),crisis_pca$eig)
crisis_Table[,1] <- NULL

Scree plots:

show_data = data.frame(pre_crisis_Table)
m1<-gvisTable(show_data,options=list(width=1920, height=min(400,27*(nrow(show_data)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m1,'chart')

show_data = data.frame(crisis_Table)
m1<-gvisTable(show_data,options=list(width=1920, height=min(400,27*(nrow(show_data)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m1,'chart')


5. 
In bull years less components are needed to explain the variance. This means that the assets are more correlated. Perhaps because the same asymmetric shocks, such as lack of trust and financial pressures affect them. 
...
The first row creates a matrix that has slot for each day and future of futures_data. Total of 245504 slots. It assigns them all to zero.

The loop then sums 0-13 moving averages for each slot in the new matrix. Because the smallest number in MAfreq is 10, the first nine rows of remain 0. The sign-function converts each of the averages into -1 or +1 depending on the whether the return was positive or negative. The signs are then summed for each slot. For instance corn has negative returns on both 10 day and 30 day period at 2001/2/16 and gets total of -2. Since this date is 34 days after start the other moving averages don't work yet.

Rest of the code uses these signals to invest 1 dollar each day based on 2 days lagged signals.
